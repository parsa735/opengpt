...
2. `OAuthService.buildAuthorizationUrl()` creates a PKCE verifier/challenge and random state, then stores one pending login in memory.
...
6. `OAuthService` atomically consumes the pending login, verifies the state, exchanges the code, extracts account/residency JWT claims, and saves the result.
...
There can be only one pending OAuth attempt because `OAuthService` stores it in an `AtomicReference`. This matches the single-user design.

Current OAuth-state caveats:

- Starting a second login replaces the first pending verifier/state.
- `PendingOAuth.createdAt` is recorded but never checked, so pending logins do not expire.
...
- `GET /auth/login`: public OAuth start; starts the callback listener and redirects to OpenAI.
- `GET /auth/callback`: fallback callback handler on the main server. The registered flow normally reaches the separate port-1455 listener.
...
- `PendingOAuth`: in-memory PKCE/state record for one login attempt.
- `TokenResponse`: OAuth token endpoint DTO; ignores unknown fields.
- `OAuthService`: builds authorization URLs, verifies callback state, exchanges authorization codes, refreshes tokens, extracts metadata, and persists credentials.
- Token exchange calls are currently blocking (`Mono.block()`) within the servlet-oriented application.
...
- Change with: `OAuthProperties`, `PkceGenerator`, `JwtParser`, `TokenStore`, callback handling, and focused OAuth tests.
...
- `OAuthController`: Spring MVC controller for login, fallback callback, auth status, local-key regeneration, and the inline HTML/CSS/JavaScript home UI.
...
- full OAuth service/callback HTTP exchanges;
...
- `auth/OAuthService.kt`.
- `auth/OAuthCallbackServer.kt`.
- `auth/OAuthController.kt`.
- `auth/JwtParser.kt` for changed token claims.
- auth tests and README setup instructions.

Do not casually change callback port/path or client ID.
